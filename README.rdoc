...
Authors::   Matt Zukowski <matt AT roughest DOT net> and Matt Campbell <matt AT soupmatt DOT com>; 
inspired by code by Ola Bini <ola.bini AT ki DOT se> and Matt Walker <mwalker AT tamu DOT edu>
            Sinatra Add on By Dan Garland <dan AT dangarland DOT co DOT uk>
...
==== Using RubyCAS-Client in Sinatra / Padrino

To run with Sinatra, add the following to your Gemfile:

gem "rubycas-client", :git => "git://github.com/dmgarland/rubycas-client"
bundle install

Then your Sinatra app could look something like this:
  ...
  class AuthServer < Sinatra::Base
    ...
    configure do
      set :config, { :cas_base_url => "https://cas.example.org", :logger => $LOGGER, 
    ...
    protect_with_cas '/'  
    get '/' do
      "Only CAS Authenticated people can see me!
    end
  end

For Padrino apps you can also do the following:

protect_with_cas :index
  
get :index do
  "Only CAS Authenticated people can see me!
end

This will also work with routes that have params, so for example
  
get :index, :with => :user do
  "Only CAS Authenticated people can see me!
end

Would still only require protect_with_cas :index
